...
LCH operator*(float scalar, const LCH &a) { return a * scalar; }
...
RGB hslToRgb(const HSL &hsl) {
  ...
  return {static_cast<uint8_t>(r_f * 255.0f),
          static_cast<uint8_t>(g_f * 255.0f),
          static_cast<uint8_t>(b_f * 255.0f)};
}
...
RGB hsvToRgb(float h, float s, float v) {
  ...
  return {static_cast<uint8_t>((r_f + m) * 255.0f),
          static_cast<uint8_t>((g_f + m) * 255.0f),
          static_cast<uint8_t>((b_f + m) * 255.0f)};
}
...
LCH rgbToLch(const RGB &rgb) {
  ...
  if (max_val != min_val) {
    ...
    c = l > 0.5f ? delta / (2.0f - max_val - min_val)
