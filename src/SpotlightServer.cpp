...
/**
 * @brief Gets an integer value from a web server argument, with a default value.
 * @param name The name of the argument.
 * @param defaultValue The value to return if the argument is not found.
...
void SpotlightServer::handleSetWheelMode() {
    ...
    String directionStr = _server.hasArg("direction") ? _server.arg("direction") : "clockwise";
    Spotlight::RotationDirection direction = Spotlight::RotationDirection::Clockwise;
...
void SpotlightServer::handleSetCycleMode() {
    ...
    bool isRandom = _server.hasArg("random") && _server.arg("random").equalsIgnoreCase("true");
...
void SpotlightServer::handleSetCycleEasing() {
    String easingStr = _server.hasArg("easing") ? _server.arg("easing") : "linear";
...
void SpotlightServer::handleSetTransitionEasing() {
    String easingStr = _server.hasArg("easing") ? _server.arg("easing") : "cubic-in-out";
    Easing::EasingFunction easing = Easing::easingFromString(easingStr);
...
void SpotlightServer::begin() {
    ...
    _server.on("/kelvin", HTTP_GET, std::bind(&SpotlightServer::handleSetKelvin, this));
    _server.on("/wheel", HTTP_GET, std::bind(&SpotlightServer::handleSetWheelMode, this));
    _server.on("/cycle", HTTP_GET, std::bind(&SpotlightServer::handleSetCycleMode, this));
    _server.on("/setCycleDuration", HTTP_GET, std::bind(&SpotlightServer::handleSetCycleDuration, this));
    _server.on("/setCycleEasing", HTTP_GET, std::bind(&SpotlightServer::handleSetCycleEasing, this));
    _server.on("/setTransitionDuration", HTTP_GET, std::bind(&SpotlightServer::handleSetTransitionDuration, this));
    _server.on("/setTransitionEasing", HTTP_GET, std::bind(&SpotlightServer::handleSetTransitionEasing, this));
